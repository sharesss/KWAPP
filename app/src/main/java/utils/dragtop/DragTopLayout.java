...
public class DragTopLayout extends FrameLayout {
    ...
    private InterceptState interceptState = InterceptState.STATE_NORMAL;
...
//        	logger.i(" ---  onViewPositionChanged  --- ");
            contentTop = top;
            requestLayout();
...
//        	logger.i(" onInterceptTouchEvent ");
        	interceptState = InterceptState.STATE_NORMAL;
        	if(panelState != PanelState.COLLAPSED){
//        		logger.i(" != PanelState.COLLAPSED ");
				...
				case MotionEvent.ACTION_DOWN:
		        	...
		        	interceptState = InterceptState.STATE_DOWN;
					break;
				case MotionEvent.ACTION_MOVE:
					...
					if(interceptState != InterceptState.STATE_MOVE)
						interceptState = InterceptState.STATE_MOVE;
//					logger.i(" onInterceptTouchEvent Move : "+interceptState);
					
					if (xDistance < yDistance) {
						logger.i("xDistance < yDistance !!");
						return true;
					}
				case MotionEvent.ACTION_UP:
		        	interceptState = InterceptState.STATE_UP;
					break;
	        	}
	        	
	        	dragHelper.shouldInterceptTouchEvent(ev);
//	        	logger.i(" dragHelper.shouldInterceptTouchEvent(ev) ");
	        	return false;
        	}
        	
        	boolean result = shouldIntercept && dragHelper.shouldInterceptTouchEvent(ev);
//        	logger.i("ShouldIntercept : "+shouldIntercept +" result : "+result);
            return result;
        } catch (NullPointerException e) {
            logger.w(e);
        }
//        logger.i(" NullPointer onInterceptTouchEvent ");
        return false;
    }
...
//    	logger.i(" onTouchEvent " + interceptState + " onTouchEvent Action : "+event.getAction());
    	
    	if(interceptState == InterceptState.STATE_DOWN){
    		if(event.getAction() == MotionEvent.ACTION_MOVE){
				...
				float result = curY - yLast;
				
				if(xDistance < yDistance && result < 0)
					interceptState = InterceptState.STATE_MOVE;
				else{
					interceptState = InterceptState.STATE_DOWN;
					return true;
...
//        logger.i(" dispatchingChildrenContentView : "+dispatchingChildrenContentView);
        if (!dispatchingChildrenContentView) {
            try {
...
//        logger.i(" panelState == PanelState.COLLAPSED : "+ (panelState == PanelState.COLLAPSED) +" ratio : "+ratio);
        if (action == MotionEvent.ACTION_MOVE && ratio == 0.0f && panelState == PanelState.COLLAPSED) {
    ...
    public void setContentVisibily(int visibility){
    	if(dragContentView != null)
    		dragContentView.setVisibility(visibility);
    }
    
    public View getContentView(){
    	return dragContentView;
    }
