 ...
 * 三级联动
 */
public class PopupWheelTimeView extends BaseEventPopup implements OnClickListener {
    ...
    private WheelView yearView, monthView;
    private String yearStr = "2017年", monthStr = "01月", dataStr = "01日";

    public PopupWheelTimeView(Activity context) {
        super(context);

        int year = Integer.valueOf(yearDate.format(new Date()));
        for (int i = year; i > year - 101; i--) {
        ...
        switch (v.getId()) {
            case R.id.tv_finish:
                if (mWheelListener != null) {
                    ...
                    String date = String.format("%s/%s", yearStr.substring(0, yearStr.length() - 1), monthStr.substring(0, monthStr.length() - 1));
                    mWheelListener.completeCall(date, "", 1);
                }
                break;
            case R.id.tv_clean:
