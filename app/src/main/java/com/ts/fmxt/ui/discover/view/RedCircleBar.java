...
public class RedCircleBar extends View {
	...
	public RedCircleBar(Context context) {
		super(context);
		init(null, 0);
	}

	public RedCircleBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs, 0);
	}

	public RedCircleBar(Context context, AttributeSet attrs, int defStyle) {
...
//		mColorWheelPaint.setStrokeCap(Paint.Cap.ROUND);//开启显示边缘为圆形
		...
		mDefaultWheelPaint.setColor(mWheelColor);
		mDefaultWheelPaint.setStyle(Style.STROKE);
		mDefaultWheelPaint.setStrokeWidth(dip2px(getContext(), 1));//圆圈的线条粗细
//		mDefaultWheelPaint.setStrokeCap(Paint.Cap.ROUND);//开启显示边缘为圆形
	...
	protected void onDraw(Canvas canvas) {
		canvas.drawArc(mColorWheelRectangle, -90, 360, false, mDefaultWheelPaint);//画外接的圆环
		canvas.drawArc(mColorWheelRectangle, -90, mSweepAnglePer, false, mColorWheelPaint);//画圆环
