 ...
 * 年收入
 */
public class PopupWheelCurrencyTypeView extends BaseEventPopup implements OnClickListener  {
...
//    private UserInComeEntity info;
    ...
    public PopupWheelCurrencyTypeView(Activity context, String money) {
        super(context);
        String[] array=getContext().getResources().getStringArray(R.array.currency_typeArray);
        ...
        switch (v.getId()) {
            case R.id.tv_finish:
                if (mWheelListener != null)
                    mWheelListener.completeCall(money,"", 4);

                break;
            case R.id.tv_clean:
                if (mWheelListener != null)
                    mWheelListener.clearCall(4);
                break;
