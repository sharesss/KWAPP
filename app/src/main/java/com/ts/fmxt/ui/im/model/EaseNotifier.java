 ...
 * from Hyphenate Inc.
 */
package com.ts.fmxt.ui.im.model;
 ...
 */
public class EaseNotifier implements OnResponseListener {
...
//            ReceiverUtils.sendReceiver(ReceiverUtils.IM_MSG, null);
    ...
    public void messgeRequest() {
//        MsgStatisticsRequest request = new MsgStatisticsRequest();
//        request.setOnResponseListener(this);
//        request.setRequestType(3);
//        request.setLoadMore(false);
//        request.executePost(false);
    }

    private void userInfo(String uuid) {
    ...
    public void onSuccess(BaseResponse response) {
        if (response.getStatus() == BaseResponse.SUCCEED) {
            switch (response.getRequestType()) {
                case 3:
                    msg_num = (Integer) response.getExData();
                    EMClient.getInstance().chatManager().loadAllConversations();
                    EMChatManager mChatManager = EMClient.getInstance().chatManager();
                    int IMNum = mChatManager.getUnreadMsgsCount();
                    boolean success = ShortcutBadger.applyCount(FmxtApplication.getContext(), IMNum + msg_num);
                    break;
                case 1:
//                    FMWession.getInstance().setSendGifUri("");
//                    UserInfoEntity info = (UserInfoEntity) response.getData();
//                    FMWession.getInstance().setIMOtherUserName(info.getNickName());
//                    FMWession.getInstance().setIMOtherUserPath(info.getHeadPic());
                    break;
            }
